...
| `mint-log.jsonl` on Vercel                           | Vercel's filesystem is read-only except `/tmp`. Fixed: `mintLog.ts` now writes to `/tmp/mint-log.jsonl` when `VERCEL` env var is set. Consequence: `/api/flow/pending` (unresolved mint monitor) always returns empty on Vercel since `/tmp` is per-Lambda-instance and not shared across invocations. Acceptable for hackathon; fix with external storage (Redis/DB) for production. |
