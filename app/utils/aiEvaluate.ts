...
function buildAttrMap(state: GameDataView): Map<bigint, Attributes> {
...
function buildPosMap(state: GameDataView): Map<bigint, { row: number; col: number }> {
  const map = new Map<bigint, { row: number; col: number }>();
...
export function evaluateGameState(
  ...
  const aiActiveIds: readonly bigint[] = aiIsCreator
    ? state.creatorActiveShipIds
    : state.joinerActiveShipIds;
  const enemyActiveIds: readonly bigint[] = aiIsCreator
    ? state.joinerActiveShipIds
    : state.creatorActiveShipIds;

  // Convert bigint scores to numbers for float arithmetic
  const aiScore = Number(aiIsCreator ? state.creatorScore : state.joinerScore);
  const enemyScore = Number(aiIsCreator ? state.joinerScore : state.creatorScore);
  ...
  const scoreProgress =
    maxScore > 0
      ? Math.max(aiScore, enemyScore) / maxScore
      : 0;
