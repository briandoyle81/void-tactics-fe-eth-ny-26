...
function getAttrAt(s: GameDataView, shipId: number): Attributes | undefined {
  const i = (s.shipIds as number[]).indexOf(shipId);
...
function getRemainingShips(
  ...
  return (activeIds as number[]).filter((id) => {
    const attr = getAttrAt(state, id);
    return attr && attr.hullPoints > 0 && !(movedIds as number[]).includes(id);
  });
}
...
export function applyRoundEndTicks(state: GameDataView): GameDataView {
  ...
  let creatorActive = [...state.creatorActiveShipIds] as number[];
  let joinerActive = [...state.joinerActiveShipIds] as number[];
  const activeSet = new Set<number>([...creatorActive, ...joinerActive]);
  const destroyedIds = new Set<number>();

  (state.shipIds as number[]).forEach((sid, idx) => {
