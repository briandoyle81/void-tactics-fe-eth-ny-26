...
export interface AiAction {
  ...
  targetShipId: number; // 0 when no target
...
function shipAttrIdx(s: GameDataView, shipId: number): number {
  return (s.shipIds as number[]).indexOf(shipId);
}

function getAttr(s: GameDataView, shipId: number): Attributes | undefined {
  const i = shipAttrIdx(s, shipId);
  return i === -1 ? undefined : s.shipAttributes[i];
}

function getPos(s: GameDataView, shipId: number): { row: number; col: number } | undefined {
...
function removeFromActive(s: GameDataView, shipId: number): GameDataView {
  return {
    ...s,
...
function moveShipInState(s: GameDataView, shipId: number, r: number, c: number): GameDataView {
...
function applyShootInState(s: GameDataView, attackerId: number, targetId: number): GameDataView {
...
export function generateLegalActions(
  ...
  if (attr.hullPoints === 0) {
    return [{
      ...
      actionType: ActionType.Retreat, targetShipId: 0, specialType: 0,
    }];
  }
  ...
  const disabledEnemies = enemyActiveIds
    ...
    .filter((x): x is { id: number; row: number; col: number } => x !== null);
  ...
  for (const mp of moveTargets) {
    ...
    actions.push({
      ...
      actionType: ActionType.Pass, targetShipId: 0, specialType: 0,
    });
    ...
    if (equippedSpecial === 1) {
      ...
      actions.push({
        shipId, row: mp.row, col: mp.col,
        actionType: ActionType.Special, targetShipId: 0, specialType: 3,
      });
    }
  }
