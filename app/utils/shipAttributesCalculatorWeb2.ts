...
function calcBaseHullPoints(ship: Web2Ship): number {
...
function calcBaseMovement(ship: Web2Ship): number {
...
function calcBaseDamageReduction(ship: Web2Ship): number {
  const armor = ATTR_ARMORS[ship.equipment.armor] ?? ATTR_ARMORS[0];
  const shield = ATTR_SHIELDS[ship.equipment.shields] ?? ATTR_SHIELDS[0];
...
export function calculateAttributesFromContractsWeb2(ship: Web2Ship): Attributes {
  const gun = ATTR_GUNS[ship.equipment.mainWeapon] ?? ATTR_GUNS[0];
