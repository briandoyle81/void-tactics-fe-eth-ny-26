...
function dbLobbyToLobby(db: {
  ...
  const statusMap: Record<string, Web2LobbyStatus> = {
    OPEN: Web2LobbyStatus.Open,
    FLEET_SELECTION: Web2LobbyStatus.FleetSelection,
    IN_GAME: Web2LobbyStatus.InGame,
    CANCELLED: Web2LobbyStatus.Open,
    COMPLETED: Web2LobbyStatus.InGame,
  };
  return {
    basic: {
      id: db.id,
    ...
    players: {
      joiner: db.joinerId ?? "",
      reservedJoiner: db.reservedJoinerId ?? "",
    ...
    state: {
      status: statusMap[db.status] ?? Web2LobbyStatus.Open,
      gameStartedAt: 0,
    },
  };
}
...
export async function GET() {
  ...
  const lobbies = await prisma.lobby.findMany({
    ...
    select: {
      ...
      joinedAt: true, joinerFleetSetAt: true,
