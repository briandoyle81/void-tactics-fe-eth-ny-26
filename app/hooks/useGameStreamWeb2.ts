...
/**
 * Subscribe to server-sent events for a web2 game.
 * Invalidates the ["gamesWeb2", gameId] React Query cache when the server
 * pushes an update, so both players see moves within ~2s without the client
 * polling on a short interval.
 */
export function useGameStreamWeb2(gameId: number, enabled = true) {
