...
function MapPlacementsEditor({ mapId, configs }: { mapId: number; configs: AIShipConfigWeb2[] }) {
  ...
  const [configId, setConfigId] = useState<string>(configs[0]?.id.toString() ?? "");
  ...
  const handleAdd = async () => {
    ...
    setPending(true);
    try {
      await apiMutate("/api/admin/ai-map-placements", "PUT", { mapId, row, col, configId: Number(configId) });
      await queryClient.invalidateQueries({ queryKey: ["ai-map-placements", mapId] });
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Failed to set placement");
    } finally {
      setPending(false);
    }
  };
  ...
  return (
    <div className="space-y-3 border border-gunmetal bg-black/40 p-3">
      <div className="space-y-1">
        {placements.length > 0 ? (
          placements.map((p) => (
            <div key={p.id} className="flex justify-between text-xs text-text-secondary">
              <span>Row {p.row}, Col {p.col}</span>
              <span className="text-cyan">{p.config.name}</span>
            </div>
          ))
        ) : (
          <p className="text-xs text-text-muted">No placements configured for this map yet.</p>
        )}
      </div>
      <div className="grid grid-cols-3 gap-3 items-end">
        <div>
          <label className="block text-xs text-cyan mb-1">Row</label>
          <input type="number" value={row} onChange={(e) => setRow(Number(e.target.value))} className={inputClass} style={inputStyle} />
        </div>
        <div>
          <label className="block text-xs text-cyan mb-1">Col</label>
          <input type="number" value={col} onChange={(e) => setCol(Number(e.target.value))} className={inputClass} style={inputStyle} />
        </div>
        <div>
          <label className="block text-xs text-cyan mb-1">Ship Config</label>
          <select value={configId} onChange={(e) => setConfigId(e.target.value)} className={inputClass} style={inputStyle}>
            {configs.map((c) => (
